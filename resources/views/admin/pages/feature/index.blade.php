                                    ...
                                    <tbody>
                                        @foreach ($features as $feature)
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($feature->image)
                                                        <img src="{{ asset('uploads/features/' . $feature->image) }}"
                                                            alt="{{ $feature->title }}" width="80">
                                                    @else
                                                        No Image
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    @if ($feature->status == 1)
                                                        <a class="btn btn-danger waves-effect btn-circle waves-light"
                                                            href="{{ route('features.inactive', $feature->id) }}">
                                                            <i class="fa fa-eye-slash"></i> </a>
                                                    @else
                                                        <a class="btn btn-success waves-effect btn-circle waves-light"
                                                            href="{{ route('features.active', $feature->id) }}">
                                                    ...
                                                    <form hidden action="{{ route('features.destroy', $feature->id) }}"
                                                        id="form{{ $feature->id }}" method="get">
                                                        @csrf
                                                    </form>
                                                    <button class="btn btn-danger waves-effect btn-circle waves-light"
                                                        onclick="deleteItem({{ $feature->id }});" type="button">
                                                        <i class="fa fa-trash"></i> </button>
                                                </td>
                                                {{-- <td>
                                            <a href="{{ route('features.edit', $feature->id) }}" class="btn btn-sm btn-warning">Edit</a>
