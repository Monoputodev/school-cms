                                    ...
                                    <tbody>
                                        @foreach ($results as $result)
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($result->file)
                                                        <img src="{{ asset('uploads/files/' . $result->file) }}"
                                                            alt="{{ $result->title }}" width="80">
                                                    @else
                                                        No File
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    @if ($result->status == 1)
                                                        <a class="btn btn-danger waves-effect btn-circle waves-light"
                                                            href="{{ route('results.inactive', $result->id) }}">
                                                            <i class="fa fa-eye-slash"></i> </a>
                                                    @else
                                                        <a class="btn btn-success waves-effect btn-circle waves-light"
                                                            href="{{ route('results.active', $result->id) }}">
                                                    ...
                                                    <form hidden action="{{ route('results.destroy', $result->id) }}"
                                                        id="form{{ $result->id }}" method="get">
                                                        @csrf
                                                    </form>
                                                    <button class="btn btn-danger waves-effect btn-circle waves-light"
                                                        onclick="deleteItem({{ $result->id }});" type="button">
