                                    ...
                                    <tbody>
                                        @foreach ($photos as $photo)
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($photo->image)
                                                        <img src="{{ asset('uploads/photos/' . $photo->image) }}"
                                                            alt="{{ $photo->title }}" width="80">
                                                    @else
                                                        No Image
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    @if ($photo->status == 1)
                                                        <a class="btn btn-danger waves-effect btn-circle waves-light"
                                                            href="{{ route('photos.inactive', $photo->id) }}">
                                                            <i class="fa fa-eye-slash"></i> </a>
                                                    @else
                                                        <a class="btn btn-success waves-effect btn-circle waves-light"
                                                            href="{{ route('photos.active', $photo->id) }}">
                                                    ...
                                                    <form hidden action="{{ route('photos.destroy', $photo->id) }}"
                                                        id="form{{ $photo->id }}" method="get">
                                                        @csrf
                                                    </form>
                                                    <button class="btn btn-danger waves-effect btn-circle waves-light"
                                                        onclick="deleteItem({{ $photo->id }});" type="button">
                                            ...
                                            <form action="{{ route('photos.destroy', $photo->id) }}" method="POST" style="display: inline-block;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this photo?')">Delete</button>
                                            </form>
                                        </td> --}}
                                            </tr>
                                        @endforeach
